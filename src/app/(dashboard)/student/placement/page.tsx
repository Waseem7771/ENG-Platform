  ...
  if (error || !data) {
    return (
      <div className="mx-auto max-w-lg rounded-2xl border border-red-500/20 bg-red-500/5 p-8 text-center">
        <p className="text-white/70">{error ?? "Couldn't load the placement exam."}</p>
        ...
        <div className="rounded-2xl border border-white/5 bg-white/[0.02] p-7">
          ...
          {data.taken && data.lastResult && (
            <div className="mt-6 rounded-xl border border-white/10 bg-white/[0.02] p-5">
              <p className="text-xs uppercase tracking-wider text-white/30">Your last result</p>
      ...
      <div className="mx-auto max-w-2xl space-y-6">
        <div>
          <div className="mb-2 flex justify-between text-xs text-white/30">
            ...
            <span className="rounded-full border border-white/10 px-2 py-0.5 uppercase tracking-wider">{question.category}</span>
          </div>
        ...
        {question.passage && (
          <div className="rounded-2xl border border-white/5 bg-white/[0.02] p-5 text-sm leading-relaxed text-white/60">{question.passage}</div>
        )}

        <p className="text-lg font-medium tracking-tight text-white">{question.question}</p>
          ...
          {question.options.map((opt) => (
            <button
              ...
              className={`w-full rounded-xl border px-4 py-3 text-left text-sm font-medium transition-all duration-200 ${
                answers[question.id] === opt ? "border-violet-500/50 bg-violet-500/10 text-white" : "border-white/10 bg-white/[0.02] text-white/80 hover:border-white/20"
              }`}
            >
              {opt}
            </button>
        ...
        <div className="flex justify-between gap-3">
          <button
            ...
            className="rounded-full border border-white/15 px-6 py-2.5 text-sm text-white/70 disabled:opacity-30"
          >
            Back
          </button>
          <button
            ...
            className="rounded-full bg-gradient-to-r from-violet-600 to-blue-500 px-8 py-2.5 text-sm font-medium uppercase tracking-wider text-white disabled:opacity-30"
          >
    ...
    return (
      <div className="mx-auto max-w-2xl space-y-8 text-center">
        ...
        <div className="flex flex-col items-center gap-4">
          ...
          <span className="rounded-full border border-amber-500/30 bg-amber-500/10 px-3 py-1 text-xs font-semibold text-amber-300">+50 XP completion bonus</span>
        </div>
  ...
  return (
    <div className="rounded-xl border border-white/5 bg-white/[0.02] p-4 text-left">
      ...
      <div className="h-2 w-full overflow-hidden rounded-full bg-white/5">
        <div className="h-full rounded-full bg-gradient-to-r from-violet-500 to-cyan-500 transition-all duration-700" style={{ width: `${value}%` }} />
      </div>
    </div>
  );
}
...
function PlacementSkeleton() {
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <div className="h-80 animate-pulse rounded-2xl border border-white/5 bg-white/[0.02]" />
    </div>
  );
}
