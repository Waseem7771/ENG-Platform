...
export function StudentDetailDialog({ studentId, onOpenChange }: { studentId: string | null; onOpenChange: (open: boolean) => void }) {
  ...
  return (
    <Dialog open={studentId !== null} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] overflow-y-auto border border-border bg-popover text-foreground sm:max-w-xl">
        {loading && (
        ...
        {!loading && !error && detail && (
          <>
            <DialogHeader>
              ...
              <DialogDescription className="text-muted-foreground">{detail.student.email}</DialogDescription>
            </DialogHeader>
            ...
            <div className="rounded-2xl border border-border bg-card p-5">
              ...
              {detail.placements.length === 0 ? (
                <p className="text-sm text-muted-foreground">No placement exams taken yet.</p>
              ) : (
                  ...
                  {detail.results.map((r) => (
                    <li key={r.id} className="flex items-center justify-between gap-3 rounded-xl border border-border bg-card px-3.5 py-2.5 text-sm">
                      <div className="min-w-0">
                        <p className="truncate font-medium text-foreground">{r.exercise.title}</p>
